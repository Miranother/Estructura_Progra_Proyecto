...
bool compararPorCosto(const Datos &a, const Datos &b) {
...
void Mercancia::ordenarPorCosto() {
    for (int i = 0; i < tipos_mercancias - 1; ++i) {
        int maxIndex = i;
        for (int j = i + 1; j < tipos_mercancias; ++j) {
            if (merca[j].getCosto() > merca[maxIndex].getCosto()) {
                maxIndex = j;
            }
        }
        swap(merca[i], merca[maxIndex]);
    }
}
...
void Mercancia::calcularEmbarque() {
    ...
    for (size_t i = 0; i < merca.size(); ++i) {
        const Datos& mercancia = merca[i];
