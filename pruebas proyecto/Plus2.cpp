...
void Mercancia::calcularCargaOptima() {
    ...
    // Imprimir para depuración: valor por volumen de cada mercancía
    cout << "Valor por volumen de las mercancias:" << endl;
    for (const auto& vp : valorPorVolumen) {
        int idx = vp.second;
        cout << "Mercancia: " << merca[idx].getNombre() << ", Valor por volumen: " << vp.first << ", Indice: " << idx << endl;
    }
